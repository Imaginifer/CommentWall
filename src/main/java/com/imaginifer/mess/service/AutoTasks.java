...
public class AutoTasks {
    ...
    private void checkPermits(){
        if(cr.noPermits()){
            cr.addNewPermit(new Permit("ROLE_ADMIN"));
    ...
    private void addAdmin(){
        if(cr.noAdmin()){
            cr.registerNew(new Commenter("admin",pwd.encode("pa ss wo rd 12 34"),""
                    ,LocalDateTime.of(1980, 1, 1, 9, 15),cr.getPermitByName("ROLE_ADMIN")));
        }
    }
    
    private List<Commenter> fillerUsers(){
        List<Commenter> comm = new ArrayList<>();
        comm.add(new Commenter("Tank Aranka", pwd.encode("qwertzuiop"), "",
                LocalDateTime.of(1980, 1, 2, 12, 15),cr.getPermitByName("ROLE_USER")));
        comm.add(new Commenter("Citad Ella", pwd.encode("qwertzuiop"), "",
                LocalDateTime.of(1980, 1, 2, 12, 35),cr.getPermitByName("ROLE_USER")));
        comm.add(new Commenter("Techno Kolos", pwd.encode("qwertzuiop"), "",
                LocalDateTime.of(1980, 1, 2, 14, 45),cr.getPermitByName("ROLE_USER")));
        comm.add(new Commenter("Feles Elek", pwd.encode("qwertzuiop"), "",
